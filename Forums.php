...
<h2> Welcome to Forums and Disscussion! Click on any of the threads below or create a new one. </h2>
                  
				<?php
				if(isset($_SESSION['username']))
                 {echo" <form action=\"Forums.php\" method=\"POST\" enctype=\"multipart/form-data\" > ";
                   
                      echo" <h3>Create a new Thread </h3>  ";
			...
			<?php
				...
				if(isset($_POST['thr']))
					{
						...
						$sql4 = "INSERT INTO Threads (heading,user_id,date) values ('$mes','$id','$today')";
						mysqli_query($db_handle,$sql4);
						$sql4 = "INSERT INTO Discussion (heading,comment,user_id,date) values ('$mes','$mes','$id','$today')";
						mysqli_query($db_handle,$sql4);

					}
				$sql = "Select * from threads order by headId";
                $res=mysqli_query($db_handle,$sql);
				$count = 0;
				...
				while($db_field = mysqli_fetch_assoc($res))
					{
						print("   <a href=\"Threads.php?data=");
						print($db_field['heading']);
						print("\">");
						$count++;
						echo "<h3>".$count.". ".$db_field['heading']."</h3>";
